              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    {
                    ?>
                  <tr id="driver<?php echo $user->user_id;?>">
                    <td><?php echo $i;?></td>
                    ...
                    <td>
                        <a href="javascript:void()" class="btn-danger" onclick="delete_driver(<?php echo $user->user_id;?>)"><i class="fas fa-trash-alt"></i></a>
                        &nbsp;&nbsp; <a href="<?php echo base_url(); ?>drivers/view-driver/<?php echo $user->user_id;?>" class="btn-info"><i class="fas fa-eye"></i></a>
                    </td>
                  </tr>
                  <?php 
                  $i++;
...
function delete_driver(id) {
  if (confirm("you are about to delete a record. This cannot be undone. are you sure?")) {
     var id = id;
        $('#hide'+id).show();
        $.ajax({
            url: "<?php echo base_url(); ?>drivers/delete_driver/"+id,
            type: 'get',
            success: function () {
          $('#driver'+id).hide();
          
            window.location.href="<?php echo base_url(); ?>drivers";
            toastr.error('Driver Deleted Successfully')
            return false;
            },
            error: function () {
                alert('ajax failure');
    ...
    $(".user_status").bootstrapSwitch();
     ...
     $('.user_status').on('switchChange.bootstrapSwitch', function (e, data) {
      ...
      if((status != '' || status != null) && (id !='')) {
          $.ajax({
              type: 'POST',
              url: "<?php echo base_url(); ?>drivers/change_user_status",
              data: {id: id,status: status},
              success: function() {
              	if(status==1){
              		bootbox.alert("Driver Status Changed to ON");
              	}else{
              		bootbox.alert("Driver Status Changed to OFF");
              	}
